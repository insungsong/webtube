...
import passportLocalMongoose from "passport-local-mongoose"; // passport-local-mongoose는 해당 쿠키에 id를 주고 id의 사용자를 판별하는 기능은 물론, 여러 암호화 기능등 로그인에 필요한 다양한 기능을 수행한다.
...
UserSchema.plugin(passportLocalMongoose, { usernameField: "email" }); // usernameField로 설정할 것은, form을 해서 보내는, name 즉 value와 값이 같아야한다.
...
// model 생성!
const model = mongoose.model("User", UserSchema); // User라는 이름의 model이고 형태는 UserSchema라는 뜻
